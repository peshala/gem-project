 ...
 * @return string
 */
function bryte_get_main_menu() {
	$args = apply_filters( 'bryte_main_menu_args', array(
		'theme_location'   => 'main',
		'container'        => '',
		'menu_id'          => 'main-menu',
		'echo'             => false,
		'fallback_cb'      => 'bryte_set_nav_menu',
		'fallback_message' => esc_html__( 'Set main menu', 'bryte' ),
	) );
 ...
 * @return void
 */
function bryte_main_menu() {

	$main_menu = bryte_get_main_menu();
 ...
 * @return void
 */
function bryte_vertical_main_menu( $slide = 'left', $args = array() ) {

	$default_args = apply_filters( 'bryte_vertical_menu_args', array(
		...
		'button-back-inner-html'  => '<i class="nc-icon-mini arrows-1_minimal-left"></i><span class="navigation-button__text">' . esc_html__( 'Back', 'bryte' ) . '</span>',
		'button-close-inner-html' => '<i class="nc-icon-mini ui-1_simple-remove"></i>',
	) );

	$args        = wp_parse_args( $args, $default_args );
	$menu        = bryte_get_main_menu();
 ...
 * @return void
 */
function bryte_footer_menu() {
	if ( ! get_theme_mod( 'footer_menu_visibility', bryte_theme()->customizer->get_default( 'footer_menu_visibility' ) ) ) {
		return;
	}

	$args = apply_filters( 'bryte_footer_menu_args', array(
		...
		'echo'             => false,
		'fallback_cb'      => 'bryte_set_nav_menu',
		'fallback_message' => esc_html__( 'Set footer menu', 'bryte' ),
	) );
 ...
 * @return void
 */
function bryte_top_menu() {
	
	if ( ! get_theme_mod( 'top_menu_visibility', bryte_theme()->customizer->get_default( 'top_menu_visibility' ) ) ) {
		return;
	}
	...
	wp_nav_menu( apply_filters( 'bryte_top_menu_args', array(
		'theme_location'  => 'top',
		'container'       => 'div',
 ...
 * @return string
 */
function bryte_get_social_list( $context, $type = 'icon' ) {
	static $instance = 0;
	$instance++;
	...
	$args = apply_filters( 'bryte_social_list_args', array(
		...
		'echo'             => false,
		'fallback_cb'      => 'bryte_set_nav_menu',
		'fallback_message' => esc_html__( 'Set social menu', 'bryte' ),
	), $context, $type );
 ...
 * @return null|string
 */
function bryte_set_nav_menu( $args ) {
 ...
 * @return void
 */
function bryte_menu_toggle( $menu_id ) {
	$format = apply_filters(
		'bryte_menu_toggle_html',
 ...
 * @return void
 */
function bryte_vertical_menu_toggle( $menu_id ) {
	$format = apply_filters(
		'bryte_vertical_menu_toggle_html',

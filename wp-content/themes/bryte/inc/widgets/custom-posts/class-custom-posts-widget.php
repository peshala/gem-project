...
if ( ! class_exists( 'Bryte_Custom_Posts_Widget' ) ) {

	/**
	 * Class Bryte_Custom_Posts_Widget.
	 */
	class Bryte_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Custom Posts', 'bryte' );
			$this->widget_description = esc_html__( 'Display custom posts your site.', 'bryte' );
			$this->widget_id          = apply_filters( 'bryte_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass    = apply_filters( 'bryte_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility            = bryte_utility()->utility;
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Custom Posts', 'bryte' ),
				...
				'terms_type' => array(
					...
					'options' => array(
						...
						'post_format'   => array(
							'label' => esc_html__( 'Post Format', 'bryte' ),
				...
				'post_format' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select post format', 'bryte' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select post format', 'bryte' ),
				...
				'excerpt_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'bryte' ),
				),
				'meta_data' => array(
					'type'    => 'checkbox',
					'value'   => array(
					...
					'options' => array(
						...
						'more_button'   => esc_html__( 'More Button', 'bryte' ),
					),
				...
				'button_text' => array(
					'type'  => 'text',
					'value' => 'Read More',
					'label' => esc_html__( 'Post read more button label', 'bryte' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'bryte_custom_posts_widget_view_dir', 'inc/widgets/custom-posts/views/custom-post-view.php' ), false, false );
			...
			$footer_columns = get_theme_mod( 'footer_widget_columns', bryte_theme()->customizer->get_default( 'footer_widget_columns' ) );
	...
	add_action( 'widgets_init', 'bryte_register_custom_posts_widget' );
	/**
	 * Register custom-posts widget.
	 */
	function bryte_register_custom_posts_widget() {
		register_widget( 'Bryte_Custom_Posts_Widget' );
	}
}

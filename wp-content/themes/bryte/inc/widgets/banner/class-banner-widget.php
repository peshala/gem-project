...
if ( ! class_exists( 'Bryte_Banner_Widget' ) ) {

	/**
	 * Class Bryte_Banner_Widget.
	 */
	class Bryte_Banner_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display a banner in your sidebar.', 'bryte' );
			$this->widget_id          = 'bryte_widget_banner';
			$this->widget_name        = esc_html__( 'Banner', 'bryte' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => '',
				...
				'media_size' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select image size', 'bryte' ),
					'placeholder'      => esc_html__( 'Select image size', 'bryte' ),
				),
				'link' => array(
					'type'        => 'text',
					'placeholder' => esc_html__( 'Type a banner`s link', 'bryte' ),
					'value'       => esc_url( home_url( '/' ) ),
					'label'       => esc_html__( 'Link:', 'bryte' ),
				),
				'target' => array(
					'type'    => 'select',
					'options' => array(
						'_blank' => esc_html__( 'A new window or tab', 'bryte' ),
						'_self'  => esc_html__( 'The same frame as it was clicked', 'bryte' ),
					),
					'value' => '_blank',
					'label' => esc_html__( 'Opens in:', 'bryte' ),
				),
			);
		...
		public function get_image_sizes() {
			...
			$result = array_merge( $result, array( 'full' => esc_html__( 'Full', 'bryte' ) ) );
...
add_action( 'widgets_init', 'bryte_register_banner_widget' );
/**
 * Register banner widget.
 */
function bryte_register_banner_widget() {
	register_widget( 'Bryte_Banner_Widget' );
}

...
if ( ! class_exists( 'Bryte_Featured_Posts_Block_Widget' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class Bryte_Featured_Posts_Block_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Featured Posts Block', 'bryte' );
			$this->widget_description = esc_html__( 'This widget displays latest posts', 'bryte' );
			$this->widget_id          = 'bryte_widget_featured_posts_block';
			...
			foreach( $layouts as $id => $layout ) {
				$layout_options[ $id ] = array(
					'label'   => $layout['name'],
					'img_src' => BRYTE_THEME_URI . '/assets/images/admin/widgets/featured-posts-block/' . $id . '.svg',
				);
			}
			...
			$this->settings = array(
				...
				'layout' => array(
					...
					'label'   => esc_html__( 'Layout', 'bryte' ),
					'options' => $layout_options,
				),
				'posts_ids' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Posts IDs (Optional)', 'bryte' ),
				),
				'checkboxes'     => array(
					'type'  => 'checkbox',
					'label' => esc_html__( 'Post Meta', 'bryte' ),
					'value' => array(
						'title'      => 'true',
						'excerpt'    => 'true',
						'categories' => 'true',
					...
					'options' => array(
						'title'      => esc_html__( 'Show title', 'bryte' ),
						'excerpt'    => esc_html__( 'Show excerpt', 'bryte' ),
						'categories' => esc_html__( 'Show categories', 'bryte' ),
						'tags'       => esc_html__( 'Show tags', 'bryte' ),
						'author'     => esc_html__( 'Show author', 'bryte' ),
						'date'       => esc_html__( 'Show date', 'bryte' ),
					),
				),
				'title_length' => array(
					'type'      => 'stepper',
					'value'     => 12,
					'min_value' => 1,
					'label'     => esc_html__( 'Title length (chars)', 'bryte' ),
				),
				'excerpt_length' => array(
					'type'      => 'stepper',
					'value'     => 15,
					'min_value' => 1,
					'label'     => esc_html__( 'Excerpt length (words)', 'bryte' ),
				),
			);
		...
		public function render_layout( $options = array() ) {
			...
			$item_template = locate_template(  apply_filters( 'bryte_featured_posts_block_widget_view_dir', 'inc/widgets/featured-posts-block/views/item.php' ), false, false );
			 ...
			 */
			$query = apply_filters( 'bryte_featured_posts_block_query', $query, $this->instance );
	...
	add_action( 'widgets_init', 'bryte_register_featured_posts_block_widget' );

	if ( false === function_exists( 'bryte_register_featured_posts_block_widget' ) ) {
		/**
		 * Register featured posts block widget.
		 */
		function bryte_register_featured_posts_block_widget() {
			register_widget( 'Bryte_Featured_Posts_Block_Widget' );
		}
	}
}

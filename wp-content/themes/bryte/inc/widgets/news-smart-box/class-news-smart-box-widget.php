...
/**
 * Class Bryte_News_Smart_Box_Widget
 */
class Bryte_News_Smart_Box_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_id          = 'bryte_widget_news_smart_box';
		$this->widget_name        = esc_html__( 'News Smart Box', 'bryte' );
		$this->widget_description = esc_html__( 'News Smart Box', 'bryte' );
		$this->utility            = bryte_utility()->utility;
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'News Smart Box', 'bryte' ),
			...
			'layout_type' => array(
				...
				'options' => array(
					'layout_type_1' => array(
						'label'   => esc_html__( 'Layout type 1', 'bryte' ),
						'img_src' => BRYTE_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-1.svg',
					),
					'layout_type_2' => array(
						'label'   => esc_html__( 'Layout type 2', 'bryte' ),
						'img_src' => BRYTE_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-2.svg',
					),
					'layout_type_3' => array(
						'label'   => esc_html__( 'Layout type 3', 'bryte' ),
						'img_src' => BRYTE_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-3.svg',
					),
				),
				'label' => esc_html__( 'Choose layout type', 'bryte' ),
			),
			'terms_type' => array(
				'type'    => 'radio',
				'value'   => 'category',
			...
			'categories' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'category', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary categories', 'bryte' ),
				'multiple'         => true,
				'placeholder'      => esc_html__( 'Select categories', 'bryte' ),
				'master'           => 'terms_type_category',
			),
			'current_tag' => array(
				'type'             => 'select',
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select tag', 'bryte' ),
				'placeholder'      => esc_html__( 'Select tag', 'bryte' ),
			...
			'tags' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary tags', 'bryte' ),
				'multiple'         => true,
			...
			'current_title_visibility' => array(
				...
				'label' => esc_html__( 'Display current navigation title', 'bryte' ),
			),
			'date_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display date', 'bryte' ),
			),
			'author_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display author', 'bryte' ),
			),
			'comment_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display comments', 'bryte' ),
			),
		...
		add_filter( 'bryte_theme_script_variables', array( $this, 'add_new_smart_box_nonce_var' ) );
	...
	public function add_new_smart_box_nonce_var( $vars = array() ) {
		$vars['new_smart_box_nonce'] = wp_create_nonce( 'bryte-new-smart-box-nonce' );
	...
	public function get_query_items( $query_args = array() ) {
		...
		$defaults_query_args = apply_filters( 'bryte_news_smart_box_default_query_args', array(
			'posts_per_page' => '5',
			'post_type'      => 'post',
			'orderby'        => 'date',
			'order'          => 'DESC',
	...
	public function get_smart_box_loop( $posts_query ) {
		...
		if ( ! $posts_query->have_posts() ) {
			return '<div class="container"><h4>' . esc_html__( 'Posts not found', 'bryte' ) . '</h4></div>';
		}
	...
	public function new_smart_box_instance(){
		if ( ! isset( $_GET['nonce'] ) || ! wp_verify_nonce( $_GET['nonce'], 'bryte-new-smart-box-nonce' ) ) {
			exit();
		}
...
add_action( 'widgets_init', 'bryte_register_news_smart_box_widgets' );
/**
 * Register news-smart-box widget.
 */
function bryte_register_news_smart_box_widgets() {
	register_widget( 'Bryte_News_Smart_Box_Widget' );
}

 ...
 * @package Bryte
 */
if ( ! class_exists( 'Bryte_About_Author_Widget' ) ) {

	/**
	 * Class Bryte_About_Author_Widget.
	 */
	class Bryte_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'bryte widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'bryte' );
			$this->widget_id          = 'bryte_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'bryte' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'bryte' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'bryte' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'bryte' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'bryte' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'bryte' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'bryte' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'bryte' ),
					'label' => esc_html__( 'Link label', 'bryte' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => esc_html__( 'Select a user', 'bryte' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'bryte-author-avatar' );
		...
		public function get_author_button() {
			...
			return sprintf(
				'<div class="about-author_btn_box"><a href="%2$s" class="about-author_btn %3$s">%1$s<i class="lnc-icon-mini arrows-1_minimal-right"></i></a></div>',
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'bryte_widget_about_author_view', 'inc/widgets/about-author/view/about-author.php' ), false, false );
...
add_action( 'widgets_init', 'bryte_register_about_author_widgets' );
/**
 * Register about-author widget.
 */
function bryte_register_about_author_widgets() {
	register_widget( 'Bryte_About_Author_Widget' );
}

...
if ( ! class_exists( 'Bryte_Smart_Slider_Widget' ) ) {

	/**
	 * Class Bryte_Smart_Slider_Widget.
	 */
	class Bryte_Smart_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display smart slider on your site.', 'bryte' );
			$this->widget_id          = 'bryte_widget_smart_slider';
			$this->widget_name        = esc_html__( 'Smart Slider', 'bryte' );
			...
			$this->settings           = array(
				...
				'categories' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select category', 'bryte' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select category', 'bryte' ),
				...
				'tags' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select tags', 'bryte' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select tags', 'bryte' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 1,
					'label'     => esc_html__( 'Posts count', 'bryte' ),
				),
				'post_title' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Display title', 'bryte' ),
				),
				'content' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Display content', 'bryte' ),
				),
				'more_button' => array(
					'type'   => 'switcher',
					'value'  => 'true',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Display more button', 'bryte' ),
					'toggle' => array(
						'true_toggle'  => esc_html__( 'On', 'bryte' ),
						'false_toggle' => esc_html__( 'Off', 'bryte' ),
				...
				'more_button_text' => array(
					'type'   => 'text',
					'value'  => esc_html__( 'Read more', 'bryte' ),
					'label'  => esc_html__( 'More button text', 'bryte' ),
				...
				'title_trim_words' => array(
					...
					'min_value'  => 1,
					'step_value' => 1,
					'label'      => esc_html__( 'Title words trimmed count', 'bryte' ),
				),
				'trim_words' => array(
					'type'       => 'slider',
					'value'      => 15,
					...
					'min_value'  => 1,
					'step_value' => 1,
					'label'      => esc_html__( 'Content words trimmed count', 'bryte' ),
				),
				'width' => array(
					'type'  => 'text',
					'value' => '100%',
					'label' => esc_html__( 'Slider width', 'bryte' ),
				),
				'height' => array(
					'type'  => 'text',
					'value' => '800',
					'label' => esc_html__( 'Slider height', 'bryte' ),
				),
				'orientation' => array(
					'type'    => 'select',
					'size'    => 1,
					'value'   => 'horizontal',
					'options' => array(
						'horizontal' => esc_html__( 'Horizontal', 'bryte' ),
						'vertical'   => esc_html__( 'Vertical', 'bryte' ),
					),
					'label'   => esc_html__( 'Slider orientation', 'bryte' ),
				),
				'slide_distance' => array(
					...
					'max_value' => 100,
					'min_value' => 0,
					'label'     => esc_html__( 'Slide distance(px)', 'bryte' ),
				),
				'slide_duration' => array(
					...
					'min_value'  => 100,
					'step_value' => 100,
					'label'      => esc_html__( 'Slide duration(ms)', 'bryte' ),
				),
				'slide_fade' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use fade effect?', 'bryte' ),
				),
				'navigation' => array(
					'type'   => 'switcher',
					'value'  => 'true',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Use navigation?', 'bryte' ),
					'toggle' => array(
						'true_toggle'  => esc_html__( 'On', 'bryte' ),
						'false_toggle' => esc_html__( 'Off', 'bryte' ),
				...
				'fade_navigation' => array(
					...
					'value'  => 'true',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Indicates whether the arrows will fade in only on hover', 'bryte' ),
				...
				'pagination' => array(
					...
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use pagination?', 'bryte' ),
				),
				'autoplay' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use autoplay?', 'bryte' ),
				),
				'fullScreen' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Display fullScreen button?', 'bryte' ),
				),
				'shuffle' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Indicates if the slides will be shuffled', 'bryte' ),
				),
				'loop' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use infinite scrolling?', 'bryte' ),
				),
				'thumbnails' => array(
					'type'   => 'switcher',
					'value'  => 'false',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Display thumbnails?', 'bryte' ),
					'toggle' => array(
						'true_toggle'  => esc_html__( 'On', 'bryte' ),
						'false_toggle' => esc_html__( 'Off', 'bryte' ),
				...
				'thumbnails_position' => array(
					...
					'options' => array(
						...
						'bottom' => esc_html__( 'Bottom', 'bryte' ),
						'right'  => esc_html__( 'Right', 'bryte' ),
						'left'   => esc_html__( 'Left', 'bryte' ),
					),
					'label'   => esc_html__( 'Sets the position of the thumbnail scroller', 'bryte' ),
				...
				'thumbnails_arrows' => array(
					...
					'value'  => 'true',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Display thumbnails arrows?', 'bryte' ),
					'master' => 'thumbnails_attr',
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( $posts_query ) {
				...
				$html = '<h4>' . esc_html__( 'Posts for slider not found', 'bryte' ) . '</h4>';
			}
		...
		public function render_slider( $posts_query ) {
			...
			$html = '<div class="bryte-smartslider smart-slider__instance invert" ' . $slider_html_attr . '>';
		...
		public function get_query_slider_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'bryte_smart_slider_default_query_args', array(
				'post_type'           => 'post',
				'orderby'             => 'date',
				'order'               => 'DESC',
				'posts_per_page'      => - 1,
		...
		public function get_slider_loop( $posts_query ) {
			 ...
			 * @var  array
			 */
			$title_settings = apply_filters( 'bryte_smart_slider_title_settings', array(
				'width'           => '100%%',
				'horizontal'      => '0%%',
				'vertical'        => '38%%',
			 ...
			 * @var  array
			 */
			$content_settings = apply_filters( 'bryte_smart_slider_content_settings', array(
				'width'           => '100%%',
				'horizontal'      => '5%%',
				'vertical'        => '51%%',
				'show_transition' => 'left',
			 ...
			 * @var  array
			 */
			$more_settings = apply_filters( 'bryte_smart_slider_more_button_settings', array(
				'width'           => '100%%',
				'horizontal'      => '5%%',
				'vertical'        => '58%%',
				'show_transition' => 'left',
		...
		public function get_slider_thumbnails( $posts_query ) {
			...
			while ( $posts_query->have_posts() ) : $posts_query->the_post();
				$html .= '<div class="sp-thumbnail">';
					$html .= $this->utility->media->get_image( array(
						'size'        => 'bryte-slider-thumb',
						'mobile_size' => 'bryte-slider-thumb',
	...
	add_action( 'widgets_init', 'bryte_register_smart_slider_widgets' );
	/**
	 * Register Smart slider widget.
	 */
	function bryte_register_smart_slider_widgets() {
		register_widget( 'Bryte_Smart_Slider_Widget' );
	}
}

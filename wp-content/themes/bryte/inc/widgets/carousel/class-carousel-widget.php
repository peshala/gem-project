 ...
 * @package Bryte
 */
if ( ! class_exists( 'Bryte_Carousel_Widget' ) ) {

	/**
	 * Class Bryte_Carousel_Widget.
	 */
	class Bryte_Carousel_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display a list of your posts on your site.', 'bryte' );
			$this->widget_id          = 'bryte_widget_carousel';
			$this->widget_name        = esc_html__( 'Carousel', 'bryte' );
			...
			$this->settings           = array(
				...
				'trim_title' => array(
					...
					'min_value'  => 1,
					'step_value' => 1,
					'label'      => esc_html__( 'Title chars trimmed count', 'bryte' ),
				),
				'trim_words' => array(
					'type'       => 'slider',
					'value'      => 15,
				...
				'slides_per_view' => array(
					...
					'min_value' => 1,
					'value'     => 5,
					'label'     => esc_html__( 'Number of slides per view', 'bryte' ),
				),
				'slides_per_group' => array(
					...
					'min_value' => 1,
					'value'     => 1,
					'label'     => esc_html__( 'Number slides per group', 'bryte' ),
				),
				'slides_per_column' => array(
					...
					'min_value' => 1,
					'value'     => 1,
					'label'     => esc_html__( 'Multi Row Slides Layout', 'bryte' ),
				),
				'space_between_slides' => array(
					...
					'min_value' => 0,
					'value'     => 30,
					'label'     => esc_html__( 'Width of the space between slides(px)', 'bryte' ),
				),
				'duration_speed' => array(
					...
					'min_value' => 100,
					'value'     => 500,
					'label'     => esc_html__( 'Duration of transition between slides (ms)', 'bryte' ),
				),
				'navigation' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Slider navigation', 'bryte' ),
				),
				'pagination' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Slider pagination', 'bryte' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'bryte_carousel_widget_view_dir', 'inc/widgets/carousel/views/carousel-view.php' ), false, false );
			...
			$footer_widget_columns = get_theme_mod( 'footer_widget_columns', bryte_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			$swiper_navigation_html = ( 'true' == $this->instance['navigation'] ) ? '<div id="swiper-carousel-'. $instance . '-next" class="swiper-button-next button-next"><i class="nc-icon-mini arrows-1_minimal-right"></i></div><div id="swiper-carousel-'. $instance . '-prev" class="swiper-button-prev button-prev"><i class="nc-icon-mini arrows-1_minimal-left"></i></div>' : '';
			...
			if ( $posts_query ) {
				...
				echo '<h5>' . esc_html__( 'Posts not found', 'bryte' ) . '</h5>';
			}
		...
		public function get_query_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'bryte_carousel_default_query_args', array(
				'post_type'      => 'post',
				'orderby'        => 'date',
				'order'          => 'DESC',
				'posts_per_page' => - 1,
		...
		public function get_carousel_loop( $posts_query, $template ) {
			...
			if ( ! $template  ) {
				return '<h5>' . esc_html__( 'View file not found', 'bryte' ) . '</h5>';
			}
			...
			while ( $posts_query->have_posts() ) : $posts_query->the_post();
				...
				$comments = $this->utility->meta_data->get_comment_count( array(
					...
					'sufix' => get_comments_number_text( esc_html__( 'No comment(s)', 'bryte' ), esc_html__( '1 comment', 'bryte' ), esc_html__( '% comments', 'bryte' ) ),
	...
	add_action( 'widgets_init', 'bryte_register_carousel_widget' );
	/**
	 * Register carousel widget.
	 */
	function bryte_register_carousel_widget() {
		register_widget( 'Bryte_Carousel_Widget' );
	}
}

...
if ( ! class_exists( 'Cherry_Db_Updater' ) ) {
	...
	class Cherry_Db_Updater {
		...
		public function __construct( $core = null, $args = array() ) {
			...
			$this->messages = array(
				'error'   => esc_html__( 'Module DB Updater init error in %s - version and slug is required arguments', 'bryte' ),
				'update'  => esc_html__( 'We need to update your database to the latest version.', 'bryte' ),
				'updated' => esc_html__( 'Update complete, thank you for updating to the latest version!', 'bryte' ),
			);
		...
		private function notice_submit( $slug = '' ) {
			...
			$label  = esc_html__( 'Start Update', 'bryte' );
			$url    = add_query_arg(
				array(
		...
		private function notice_title( $slug ) {
			...
			printf( '<strong>%1$s %2$s</strong> &#8211; ', $name, esc_html__( 'Data Update', 'bryte' ) );
		}

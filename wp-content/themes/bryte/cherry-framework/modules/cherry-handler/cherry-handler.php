	...
	class Cherry_Handler {
		...
		public function __construct( $core, $args = array() ) {
			$this->settings['sys_messages'] = array(
				'invalid_base_data' => esc_html__( 'Unable to process the request without nonce or server error', 'bryte' ),
				'no_right'          => esc_html__( 'No right for this action', 'bryte' ),
				'invalid_nonce'     => esc_html__( 'Stop CHEATING!!!', 'bryte' ),
				'access_is_allowed' => esc_html__( 'Access is allowed', 'bryte' ),
				'wait_processing'   => esc_html__( 'Please wait, processing the previous request', 'bryte' ),
			);

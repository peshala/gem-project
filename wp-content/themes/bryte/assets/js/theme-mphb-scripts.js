			...
			$( '.mphb_sc_search-wrapper' ).each( function() { // Check this custom class, moto must fix it
					...
					selector.each( function() {
						...
						if ( 'date' == type ) {
							...
							if ( $onLoadDate != '' ) {
								...
								$onLoadMonth      = '/' + bryte.months[$onLoadDate[0].replace( /^0+/, '' )];
				...
				$( 'body' ).on( 'click', '.datepick-popup .datepick-month a', function() {
					...
					if ( $checkInDate != '' ) {
						...
						$( '.month', $checkInDateBox ).text( '/' + bryte.months[$checkInDate[0].replace(/^0+/, '')] );
					...
					if ( $checkOutDate != '' ) {
						...
						$( '.month', $checkOutDateBox ).text( '/' + bryte.months[$checkOutDate[0].replace(/^0+/, '')] );
